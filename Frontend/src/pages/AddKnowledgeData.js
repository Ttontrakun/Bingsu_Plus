...
function AddKnowledgeData() {
  ...
  const handleSaveEdit = () => {
    setUploadedFiles((prev) =>
      prev.map((f) => (f.id === editingFile.id ? { ...f, content: editContent } : f)),
    );
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    showToast('ระบบยังไม่รองรับการเพิ่มไฟล์เข้า Knowledge เดิม', 'warning');
    navigate('/knowledge');
  };
  ...
  return (
    <div className='flex h-screen bg-white relative'>
      ...
      <main
        className={`flex-1 bg-white px-8 py-6 overflow-auto flex flex-col transition-all duration-300 ${isSidebarCollapsed ? 'pl-16' : ''}`}
      >
        <button
                    ...
                    <div>
                      <div
                        ...
                        onDrop={(e) => {
                          ...
                          if (files.length === 1) handleFileAdd(files[0]);
                        }}
                      >
                        <input
                          type='file'
                        ...
                        <label htmlFor='file-upload' className='cursor-pointer flex flex-col items-center gap-2'>
                          ...
                          <span className='text-xs text-red-500 font-semibold mt-2'>
                            **แนบไฟล์ได้ครั้งละ 1 ไฟล์ และขนาดไม่เกิน 5 MB**
                          </span>
                        </label>
                      </div>
                      {fileError && (
                        ...
                        >
                          <div className='flex-1 min-w-0'>
                            ...
                            <div className='flex items-center gap-2'>
                              ...
                              {file.sizeMB && <span className='text-xs text-gray-400'>({file.sizeMB} MB)</span>}
                            </div>
                          </div>
          ...
          <>
            ...
            <div className='fixed inset-0 z-50 flex items-center justify-center p-4'>
              <div className='bg-white rounded-lg shadow-2xl w-full max-w-sm p-6'>
                <h3 className='text-lg font-semibold text-gray-800 mb-4'>ลบไฟล์นี้หรือไม่?</h3>
            ...
            <div className='fixed inset-0 z-50 flex items-center justify-center p-4'>
              ...
              >
                <div className='px-6 py-4 border-b border-gray-200'>
                  <h3 className='text-lg font-semibold text-gray-800'>แก้ไขไฟล์: {editingFile.name}</h3>
                </div>
                <div className='p-6'>
                  <textarea
                    value={editContent}
