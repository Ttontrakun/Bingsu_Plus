...
export async function fileSearch(ragStoreName: string, query: string, prompt?: string): Promise<QueryResult> {
    if (!ai) throw new Error("Gemini AI not initialized");
    const promptPrefix = prompt ? `${prompt}\n\n` : '';
    const response: GenerateContentResponse = await ai.models.generateContent({
        model: 'gemini-2.5-flash',
        contents: `${promptPrefix}${query} DO NOT ASK THE USER TO READ THE MANUAL, pinpoint the relevant sections in the response itself.`,
        config: {
            tools: [
                    {
                        fileSearch: {
