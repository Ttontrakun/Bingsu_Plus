...
const runOcrExtract = async ({ buffer, fileName, contentType }) => {
  if (!OCR_API_URL) {
...
const processUploadBatch = async (batchId) => {
  ...
  for (const plan of processingPlan) {
    ...
    if (isPdf) {
      ...
      if (shouldRunOcrForPdf({ text, pageCount })) {
        try {
          setProgressMessage("Running OCR (Thai)...", session.name);
